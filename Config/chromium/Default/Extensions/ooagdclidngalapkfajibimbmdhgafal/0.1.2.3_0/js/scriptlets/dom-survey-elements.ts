...
interface DomSurveyElementsResult {
    busy: boolean;
    hiddenElementCount: number;
    surveyTime: number;
}

interface VAPIDomFilterer {
    getAllSelectors(mask: number): { declarative: string[] };
}

interface VAPI {
    domSurveyElements?: DomSurveyElementsResult;
    domMutationTime: number;
    domFilterer?: VAPIDomFilterer;
}
