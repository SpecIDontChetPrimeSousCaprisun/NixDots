...
(( ) => {
    ...
    const autoRefresh = function(root: DocumentFragment): void {
        ...
        console.warn('[uBR] noscript-spoof: autoRefresh URL parse failed', e);
        return;
        }
    ...
    const morphNoscript = function(from: Element): DocumentFragment {
        ...
        if ( span === null ) { return fragment; }
        for ( const elem of span.querySelectorAll(inHead ? 'span > *' : 'span') ) {
    ...
    for ( const sheet of document.styleSheets ) {
        try {
            ...
            console.warn('[uBR] noscript-spoof: cssRules iteration failed', e);
        }
    }
