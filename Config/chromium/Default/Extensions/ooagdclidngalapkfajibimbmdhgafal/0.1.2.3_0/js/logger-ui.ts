        ...
        for ( let i = 0; i < n; i++ ) {
            ...
            if ( rawPart.startsWith('/') && rawPart.endsWith('/') ) {
                ...
                try {
                    ...
                    console.warn('[uBR] logger-ui: invalid regex', e);
                    reStr = '';
                }
            }
            if ( reStr === '' ) {
...
const popupManager = (( ) => {
    ...
    const popupWatchdogInterval = 5000;
    ...
    const showEmbeddedPopupError = function(err) {
        ...
        if ( errEl === null ) {
            ...
            el.style.cssText =
                'position:absolute;left:0;top:0;right:0;padding:8px;' +
                'background:#a00;color:#fff;font-size:13px;z-index:999;';
    ...
    const heartbeatPopup = async function() {
        ...
        try {
            ...
            if ( win === null || win.vAPI === undefined || win.vAPI.messaging === undefined ) {
    ...
    const onMessage = function(ev) {
        ...
        if ( ev.source !== popup?.contentWindow ) { return; }
...
(( ) => {
    ...
    const collectLines = function() {
        lines.length = 0;
        const t0 = filteredLoggerEntries.length !== 0
...
const loggerSettings = (( ) => {
    ...
    vAPI.localStorage.getItemAsync('loggerSettings').then(value => {
        try {
            ...
            console.warn('[uBR] logger-ui: settings parsing failed', e);
        }
    });
