...
class FilterCompiler {
    ...
    normalizeRegexSource(s) {
        try {
            ...
            console.warn('[uBR] static-net-filtering: normalizeRegexSource failed', e);
        }
        return '';
    }
...
StaticNetFilteringEngine.prototype.transformURL = function(fctxt, out = []) {
    ...
    for ( const directive of directives ) {
        ...
        if ( re === undefined ) {
            ...
            catch (e) { console.warn('[uBR] static-net-filtering: redirect regex failed', e); continue; }
        }
        if ( re.test(before) !== true ) { continue; }
        const after = before.replace(re, cache.replacement);
        try { void new URL(after); } catch (e) { console.warn('[uBR] static-net-filtering: redirect URL invalid', e); continue; }
    ...
    for ( const directive of directives ) {
        ...
        while ( i > 0 ) {
            ...
            if ( name !== undefined ) {
                ...
                catch (e) { console.warn('[uBR] static-net-filtering: decodeURIComponent failed', value, e); }
