...
// Source: src/js/document-blocked.ts
...
(() => {
  ...
  var faIconsInit = /* @__PURE__ */ (() => {
    const svgIcons = /* @__PURE__ */ new Map([
      ...
      ["ph-readermode-text-size", { viewBox: "0 0 20 12.5", paths: [
    ...
    return function(root) {
      ...
      for (const icon of icons) {
        ...
        const details2 = svgIcons.get(name);
        if (details2 === void 0) {
          let file;
          ...
          svg.setAttribute("viewBox", details2.viewBox);
          if (details2.paths !== void 0) {
            for (const d of details2.paths) {
            ...
            const path = document.createElementNS(svgNS, "path");
            path.setAttribute("d", details2.path);
            svg.appendChild(path);
          }
        }
  ...
  var urlToFragment = (raw) => {
    try {
      ...
      console.warn("[uBR] document-blocked: urlToFragment URL parse failed", raw, e);
    }
    return raw;
  };
  ...
  (() => {
    ...
    const renderParams = function(parentNode, rawURL, depth = 0) {
      ...
      try {
        ...
        console.warn("[uBR] document-blocked: renderParams URL parse failed", rawURL, e);
        return false;
      }
  ...
  var proceedTemporary = async function() {
  ...
  var proceedPermanent = async function() {
    await messaging.send("documentBlocked", {
  ...
  lookupFilterLists().then((lists = []) => {
    ...
    const why = qs$(reason ? "template.why-reason" : "template.why").content.cloneNode(true);
    i18n.render(why);
