...
interface SetHaystackMessage {
    what: 'setHaystack';
    content: string;
}

interface DoSearchMessage {
    what: 'doSearch';
    id: number;
    pattern: string;
    flags: string;
...
type WorkerMessage = SetHaystackMessage | DoSearchMessage;
...
interface SearchThreadAPI {
    needHaystack: () => boolean;
    setHaystack: (content: string) => void;
    search: (query: SearchQuery, overwrite?: boolean) => Promise<number[]>;
    shutdown: () => void;
}
...
(function () {
    ...
    if (
        ...
        const doSearch = function(details: DoSearchMessage): number[] {
            const reEOLs = /\n\r|\r\n|\n|\r/g;
            ...
            try {
                ...
                console.warn('[uBR] codemirror search-thread: invalid regex', details.pattern, e);
                return [];
            }
        ...
        self.onmessage = function(e: MessageEvent) {
            ...
            switch ( msg.what ) {
            case 'setHaystack':
                content = (msg as SetHaystackMessage).content;
                break;

            case 'doSearch': {
                const searchMsg = msg as DoSearchMessage;
...

    {
        ...
        const pendingResponses = new Map<number, (value: number[]) => void>();
        ...
        const onWorkerMessage = function(e: MessageEvent) {
            const msg = e.data as ResponseMessage;
        ...
        const destroy = function() {
            shutdown();
            self.searchThread = undefined as unknown as SearchThreadAPI;
        };
        ...
        const needHaystack = function(): boolean {
            return worker instanceof Object === false;
        };

        const setHaystack = function(content: string) {
            init();
            worker!.postMessage({ what: 'setHaystack', content });
        };

        const search = function(query: SearchQuery, overwrite = true): Promise<number[]> {
            init();
