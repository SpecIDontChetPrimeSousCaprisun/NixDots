...
interface CodeMirrorLineHandle {
    lineNo(): number;
    text: string;
    gutterMarkers: { [key: string]: HTMLElement | null } | null;
}

interface CodeMirrorDoc {
    getEditor(): CodeMirrorEditor;
    getLine(line: number): string;
    getLineHandle(line: number): CodeMirrorLineHandle;
    getLineHandle(lineNo: number): CodeMirrorLineHandle | undefined;
    setGutterMarker(lineHandle: CodeMirrorLineHandle, gutter: string, marker: HTMLElement | null): void;
    eachLine(start: number, end: number, callback: (lineHandle: CodeMirrorLineHandle) => void): void;
    eachLine(callback: (lineHandle: CodeMirrorLineHandle) => void): void;
    lineCount(): number;
...
interface CodeMirrorEditor {
    ...
    getCursor(pos?: string): CodeMirrorCursor;
    getLine(line: number): string;
    getTokenAt(pos: CodeMirrorPos): { type: string; string: string };
    getTokenTypeAt(pos: CodeMirrorPos): string | null;
    on(event: string, handler: (cm: CodeMirrorEditor, ...args: unknown[]) => void): void;
    foldCode(pos: CodeMirrorPos | number, options?: object): void;
    setOption(option: string, value: unknown): void;
    startOperation(): void;
    endOperation(): void;
    commands: {
        foldAll?(cm: CodeMirrorEditor): void;
        unfoldAll?(cm: CodeMirrorEditor): void;
    };
}

interface CodeMirrorModeState {
    astParser: sfp.AstFilterParser;
    astWalker: sfp.AstWalker;
    currentWalkerNode: number;
    lastNetOptionType: number;
}
...
interface HintOptions {
    redirectResources?: Array<[string, { aliasOf: string; canRedirect: boolean; canInject: boolean }]>;
    preparseDirectiveEnv?: string[];
    preparseDirectiveHints?: string[];
    originHints?: string[];
}
...
const redirectNames = new Map<string, string>();
const scriptletNames = new Map<string, string>();
const preparseDirectiveEnv: string[] = [];
const preparseDirectiveHints: string[] = [];
...
CodeMirror.defineOption('trustedScriptletTokens', undefined, (cm, tokens) => {
    ...
    if (typeof (tokens as Iterable<unknown>)[Symbol.iterator] !== 'function') { return; }
...
const redirectTokenStyle = (mode: CodeMirrorModeState): string => {
...
const nodeHasError = (mode: CodeMirrorModeState): boolean => {
    return mode.astParser.getNodeFlags(
        mode.currentWalkerNode, sfp.NODE_FLAG_ERROR
    ) !== 0;
};

const reGoodRegexToken = /[^\x01%0-9A-Za-z][%0-9A-Za-z]{7,}|[^\x01%0-9A-Za-z][%0-9A-Za-z]{1,6}[^\x01%0-9A-Za-z]/;

const colorFromAstNode = (mode: CodeMirrorModeState): string => {
...
class ModeState {
    ...
    constructor() {
        ...
        self.addEventListener('trustedSource', ev => {
            const { trusted } = (ev as CustomEvent).detail;
            this.astParser.options.trustedSource = trusted;
        });
        self.addEventListener('trustedScriptletTokens', ev => {
            this.astParser.options.trustedScriptletTokens = (ev as CustomEvent).detail;
        });
    }
}
...
interface CodeMirrorMode {
    ...
    copyState(other: ModeState): ModeState;
    token(stream: CodeMirrorStream, state: ModeState): string | null;
    lineComment: string;
}
...
CodeMirror.defineOption('uboHints', null, (cm, hints: HintOptions) => {
...
function initHints(): void {
    ...
    const pickBestHints = function(cursor: CodeMirrorCursor, seedLeft: string, seedRight: string, hints: unknown[]): HintsResult | undefined {
        const seed = (seedLeft + seedRight).trim();
        const out: unknown[] = [];
        for (const hint of hints) {
            const text = hint instanceof Object
                ? (hint as { displayText?: string; text: string }).displayText || (hint as { text: string }).text
                : hint as string;
    ...
    const getOriginHints = function(cursor: CodeMirrorCursor, line: string, suffix = ''): HintsResult | undefined {
        const beg = cursor.ch;
    ...
    const getNetPatternHints = function(cursor: CodeMirrorCursor, line: string): HintsResult | undefined {
    ...
    const getNetOptionHints = function(cursor: CodeMirrorCursor, seedLeft: string, seedRight: string): HintsResult | undefined {
        ...
        const hints: string[] = [];
        for (const [text, desc] of sfp.netOptionTokenDescriptors) {
    ...
    const getNetRedirectHints = function(cursor: CodeMirrorCursor, seedLeft: string, seedRight: string): HintsResult | undefined {
        const hints: string[] = [];
    ...
    const getNetHints = function(cursor: CodeMirrorCursor, line: string): HintsResult | undefined {
    ...
    const getExtSelectorHints = function(cursor: CodeMirrorCursor, line: string): HintsResult | undefined {
        const beg = cursor.ch;
        {
    ...
    const getExtHeaderHints = function(cursor: CodeMirrorCursor, line: string): HintsResult | undefined {
        const beg = cursor.ch;
    ...
    const getExtScriptletHints = function(cursor: CodeMirrorCursor, line: string): HintsResult | undefined {
        const beg = cursor.ch;
        const matchLeft = /#\+js\(([^,]*)$/.exec(line.slice(0, beg));
        const matchRight = /^([^,)]*)/.exec(line.slice(beg));
        if (matchLeft === null || matchRight === null) { return; }
        const hints: { text: string; displayText?: string }[] = [];
    ...
    const getCommentHints = function(cursor: CodeMirrorCursor, line: string): HintsResult | undefined {
        const beg = cursor.ch;
    ...
    CodeMirror.registerHelper('hint', 'ubo-static-filtering', (cm: CodeMirrorEditor) => {
        ...
        if (astParser.isExtendedFilter()) {
            ...
            let hints: HintsResult | undefined;
...
CodeMirror.registerHelper('fold', 'ubo-static-filtering', (() => {
    const foldIfEndif = function(startLineNo: number, startLine: string, cm: CodeMirrorEditor) {
    ...
    const foldInclude = function(startLineNo: number, startLine: string, cm: CodeMirrorEditor) {
        const lastLineNo = cm.lastLine();
    ...
    return function(cm: CodeMirrorEditor, start: CodeMirrorPos) {
...

{
    ...
    let changesetTimer: number | undefined;

    const includeset = new Set<string>();
    let errorCount = 0;

    const ifendifSet = new Set<CodeMirrorLineHandle>();
    let ifendifSetChanged = false;

    const extractMarkerDetails = (doc: CodeMirrorDoc, lineHandle: CodeMirrorLineHandle): MarkerDetails | undefined => {
    ...
    const extractMarker = (lineHandle: CodeMirrorLineHandle): HTMLElement | null => {
    ...
    const markerTemplates: Record<string, MarkerTemplate> = {
        'error': {
            node: null,
            html: [
    ...
    const markerFromTemplate = (details: MarkerDetails): HTMLElement => {
        ...
        if (template.node === null) {
            ...
            template.node = document.adoptNode(qs$(doc, '.CodeMirror-lintmarker') as HTMLElement);
        }
    ...
    const addMarker = (doc: CodeMirrorDoc, lineHandle: CodeMirrorLineHandle, marker: HTMLElement | null, details: MarkerDetails): void => {
        ...
        const msgElem = qs$(marker, '.msg') as HTMLElement | null;
        if (msgElem === null) { return; }
        msgElem.textContent = details.msg;
    };

    const removeMarker = (doc: CodeMirrorDoc, lineHandle: CodeMirrorLineHandle, marker: HTMLElement): void => {
        doc.setGutterMarker(lineHandle, 'CodeMirror-lintgutter', null);
        if (marker.dataset.error === 'y') {
            errorCount -= 1;
        }
    ...
    const processIfendifs = (): void => {
        if (ifendifSet.size === 0) { return; }
        if (ifendifSetChanged !== true) { return; }
        const sortFn = (a: CodeMirrorLineHandle, b: CodeMirrorLineHandle) => a.lineNo() - b.lineNo();
        const sorted = Array.from(ifendifSet).sort(sortFn);
        const bad: CodeMirrorLineHandle[] = [];
        const stack: CodeMirrorLineHandle[] = [];
        for (const line of sorted) {
            ...
            if (fold === 'start') {
                ...
                if (stack.length !== 0) {
                    ...
                    const ifstart = extractMarker(stack.pop() as CodeMirrorLineHandle);
    ...
    const processDeletion = (doc: CodeMirrorDoc, change: CodeMirrorChange): void => {
    ...
    const processInsertion = (doc: CodeMirrorDoc, deadline: IdleDeadline, change: CodeMirrorChange): { from: number; to: number } | undefined => {
        let { from, to } = change;
        doc.eachLine(from.line, to.line, lineHandle => {
    ...
    const processChangeset = (doc: CodeMirrorDoc, deadline: IdleDeadline): void => {
        const cm = doc.getEditor();
        cm.startOperation();
        while (changeset.length !== 0) {
            const change = processInsertion(doc, deadline, changeset.shift() as { from: number; to: number });
    ...
    const processChangesetAsync = (doc: CodeMirrorDoc): void => {
    ...
    const onChanges = (cm: CodeMirrorEditor, changes: CodeMirrorChange[]): void => {
    ...
    const onBeforeChanges = (cm: CodeMirrorEditor, change: CodeMirrorChange): void => {
        const doc = cm.getDoc();
        processDeletion(doc, change);
    };

    const foldRangeFinder = (cm: CodeMirrorEditor, from: CodeMirrorPos): { from: CodeMirrorPos; to: CodeMirrorPos } | null => {
        ...
        const to: CodeMirrorPos = { line: 0, ch: 0 };
        const doc = cm.getDoc();
        let depth = 0;
    ...
    const onGutterClick = (cm: CodeMirrorEditor, lineNo: number, gutterId: string, ev: MouseEvent): void => {
    ...
    self.addEventListener('trustedScriptletTokens', ev => {
        astParser.options.trustedScriptletTokens = (ev as CustomEvent).detail;
    });
    ...
    CodeMirror.defineInitHook(cm => {
        ...
        cm.on('fold', (cm: CodeMirrorEditor, from: CodeMirrorPos) => {
            const doc = cm.getDoc();
        ...
        cm.on('unfold', (cm: CodeMirrorEditor, from: CodeMirrorPos) => {
            const doc = cm.getDoc();
            const lineHandle = doc.getLineHandle(from.line);
            const marker = extractMarker(lineHandle);
            if (marker === null) { return; }
...
{
    const selectWordAt = function(cm: CodeMirrorEditor, pos: CodeMirrorPos): { from: CodeMirrorPos; to: CodeMirrorPos } {
        const { line, ch } = pos;
    ...
    CodeMirror.defineInitHook(cm => {
        cm.setOption('configureMouse', (_cm: CodeMirrorEditor, repeat: string | unknown) => {
            return {
