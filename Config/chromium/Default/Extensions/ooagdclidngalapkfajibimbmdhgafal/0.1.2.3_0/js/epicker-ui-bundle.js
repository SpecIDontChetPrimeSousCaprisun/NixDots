  ...
  (() => {
    ...
    const buildKeywordConditionalBranches = function(baseFilter, andKeywords, orKeywords) {
      ...
      if (andKeywords.length !== 0) {
        const andBody = andKeywords.map((kw) => `:has-text(/${escapeRegexLiteral(kw)}/i)`).join("");
        const raw = `${anchor}${body}${andBody}`;
        parser.parse(raw);
        if (parser.isFilter() === false || parser.result.compiled === void 0) {
    ...
    const computeEffectivePickerFilterPayload = function(details) {
      const { baseRawFilter, keywordModeEnabled: keywordModeEnabled2, andInputRaw, orInputRaw } = details;
      if (keywordModeEnabled2 === false) {
        const compiled = reCosmeticAnchor.test(baseRawFilter) && staticFilteringParser.result ? staticFilteringParser.result.compiled : void 0;
        return {
          ok: true,
          mode: "plain",
      ...
      const rawLines = result.branches.map((b2) => b2.raw);
      const compiledLines = result.branches.map((b2) => b2.compiled);
      return {
        ok: true,
      ...
      if (keywordModeEnabled) {
        ...
        if (payload.ok) {
          ...
          pickerContentPort.postMessage({
            ...
            branches: payload.branches.map((b2) => ({ filter: b2.raw, compiled: b2.compiled }))
          });
        } else {
          $id("resultsetCount").textContent = "E";
          pickerContentPort.postMessage({
    ...
    const onCreateClicked = function() {
      ...
      if (keywordModeEnabled) {
        ...
        const finalFilters = payload.rawLines.map((raw) => hn2 + raw).join("\n");
        pickerContentPort.postMessage({
          what: "pickerCreateFilter",
          autoComment: true,
    ...
    const startPicker = function() {
      ...
      dialog.addEventListener("wheel", (ev) => {
        if (ev.target.closest("textarea") || ev.target.closest(".changeFilter")) {
          return;
        }
        ev.preventDefault();
      }, { passive: false });
