...
// https://github.com/uBlockOrigin/uBlock-issues/issues/688#issuecomment-663657508
{
    ...
    try {
        ...
        console.warn('[uBR] contentscript: context resolution failed', e);
    }
...

{
    ...
    const safeObserverHandler = function() {
        ...
        for ( const listener of getListenerIterator() ) {
            ...
            catch (e) { console.warn('[uBR] contentscript: onDOMChanged listener failed', e); }
        }
        addedNodes.length = 0;
        removedNodes = false;
        vAPI.domMutationTime = Date.now();
    ...
    const addListener = function(listener) {
        ...
        catch (e) { console.warn('[uBR] contentscript: onDOMCreated listener failed (addListener)', e); }
    ...
    const start = function() {
        for ( const listener of getListenerIterator() ) {
            try { listener.onDOMCreated(); }
            catch (e) { console.warn('[uBR] contentscript: onDOMCreated listener failed (start)', e); }
        }
        startMutationObserver();
    };
