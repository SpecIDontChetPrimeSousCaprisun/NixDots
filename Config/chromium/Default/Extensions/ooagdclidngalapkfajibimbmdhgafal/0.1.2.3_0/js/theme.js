...
function setTheme(theme, propagate = false) {
    ...
    for (;;) {
        ...
        if ( w === w.parent ) { break; }
        w = w.parent;
        try { void w.document; } catch (e) { console.warn('[uBR] theme: cross-origin frame access failed', e); return; }
    }
}
...
function setAccentColor(
    ...
    for (;;) {
        ...
        if ( w === w.parent ) { break; }
        w = w.parent;
        try { void w.document; } catch (e) { console.warn('[uBR] theme: cross-origin frame access in setAccentColor failed', e); break; }
    }
}
