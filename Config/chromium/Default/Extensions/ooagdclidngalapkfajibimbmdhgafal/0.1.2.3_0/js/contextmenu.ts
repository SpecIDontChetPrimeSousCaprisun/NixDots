...
interface ContextMenuUpdateResult {
    update: (tabId?: number) => void;
}

/******************************************************************************/

const contextMenu = ((): ContextMenuUpdateResult => {
...
const onBlockElement = function(details: ContextMenuDetails, tab?: TabInfo): void {
...
const onBlockElementInFrame = function(details: ContextMenuDetails, tab?: TabInfo): void {
    if ( tab === undefined ) { return; }
    if ( /^https?:\/\//.test(details.frameUrl || '') === false ) { return; }
    µb.epickerArgs.mouse = false;
    µb.elementPickerExec(tab.id, details.frameId || 0);
};

/******************************************************************************/

const onSubscribeToList = function(details: ContextMenuDetails): void {
    let parsedURL: URL | undefined;
    try {
        parsedURL = new URL(details.linkUrl || '');
    }
    catch (e) {
        console.warn('[uBR] contextmenu: invalid subscribe URL', details.linkUrl, e);
    }
...
const onTemporarilyAllowLargeMediaElements = function(details: ContextMenuDetails, tab?: TabInfo): void {
    if ( tab === undefined ) { return; }
...
const onViewSource = function(details: ContextMenuDetails, tab?: TabInfo): void {
    if ( tab === undefined ) { return; }
...
const onEntryClicked = function(details: ContextMenuDetails, tab?: TabInfo): void {
...
const menuEntries: Record<string, MenuEntry> = {
    blockElement: {
...
const update = function(tabId?: number): void {
    let newBits = 0;
...
vAPI.contextMenu.onMustUpdate = async function(tabId?: number): Promise<void> {
