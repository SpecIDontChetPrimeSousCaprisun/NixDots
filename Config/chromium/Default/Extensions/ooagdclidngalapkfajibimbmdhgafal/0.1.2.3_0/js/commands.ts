...
interface Timer {
    offon: (delay: number, tabId: number) => void;
}
...
const relaxBlockingMode = (( ) => {
    const reloadTimers = new Map<number, Timer>();

    return function(tab: Tab): void {
        ...
        let newProfileBits: number | undefined;
        ...
        const timer = reloadTimers.get(tab.id) || (( ): Timer => {
            const t = vAPI.defer.create((tabId: number) => {
...
vAPI.commands.onCommand.addListener(async (command: string): Promise<void> => {
