...
const gotoZap = async function() {
    ...
    try {
        ...
        console.warn('[uBR] popup-fenix: gotoZap injectZapperScripts failed', e);
    }
...
const gotoPick = async function() {
    ...
    try {
        ...
        console.warn('[uBR] popup-fenix: gotoPick launchElementPicker failed', e);
    }
...
const reloadTab = function(bypassCache = false) {
    ...
    // Reload the tab directly (MV3 compatible)
    if (popupData.tabId) {
        try {
            chrome.tabs.reload(popupData.tabId, { bypassCache: bypassCache || forceReloadFlag !== 0 });
        } catch (e) {
            // Fallback: try messaging approach
            messaging.send('popupPanel', {
                what: 'reloadTab',
                tabId: popupData.tabId,
...
vAPI.localStorage.getItemAsync('popupExpandExceptions').then(exceptions => {
    try {
        ...
        console.warn('[uBR] popup-fenix: expand exceptions load failed', e);
    }
});
...
const getPopupData = async function(tabId, first = false) {
    ...
    if ( response === undefined || response.error ) {
        showError(response?.error === 'No handler registered for channel popupPanel'
            ? popupErrorMessages.sw
...

{
    ...
    getPopupData(tabId, true).then(( ) => {
        if ( dom.cl.has(dom.body, 'loading') === false ) { return; }
