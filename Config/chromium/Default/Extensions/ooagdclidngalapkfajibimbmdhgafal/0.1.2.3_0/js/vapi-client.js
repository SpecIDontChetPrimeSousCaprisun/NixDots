...
vAPI.messaging = {
    ...
    disconnectListener: function() {
        ...
        if ( window !== window.top ) {
            // Extension-owned UI iframes (popup-fenix, logger-ui, dashboard)
            // should not permanently shut down on port disconnect — the SW
            // may have restarted. Instead, destroy the port so getPort()
            // recreates it on the next send.
            const isExtensionUIFrame =
                location.protocol === `${webextAPI.runtime.getURL('').split(':')[0]}:` &&
                /\/(?:popup-fenix|logger-ui|dashboard)\.html(?:[?#]|$)/.test(
                    location.pathname + location.search,
                );
            if ( isExtensionUIFrame ) {
                this.destroyPort();
            } else {
                vAPI.shutdown.exec();
            }
    ...
    try {
        ...
        if ( this.port === null || webextAPI?.runtime?.lastError ) {
            void webextAPI?.runtime?.lastError;
        }
    } catch (e) {
            if ( this.shuttingDown === false ) {
                console.log('[uBR] vapi-client: runtime.connect failed — extension context invalidated');
                this.shuttingDown = true;
            }
            this.port = null;
        }
