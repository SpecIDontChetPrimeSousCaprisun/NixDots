(() => {
      ...
      static onFirstShown(fn, elem) {
          let observer = new IntersectionObserver((entries) => {
              ...
              try {
                  ...
                  console.warn('[uBR] cloud-ui: observer callback fn failed', e);
              }
        observer?.disconnect();
        observer = void 0;
          });
  ...
  const faIconsInit = /* @__PURE__ */ (() => {
      ...
      return function(root) {
        ...
        if (details === void 0) {
          ...
          if ( details.paths !== void 0 ) {
              for ( const d of details.paths ) {
  ...
  (() => {
      ...
      const fetchCloudData = async function() {
        ...
        info.textContent = i18n$("cloudNoData");
        return entry;
          }
      ...
      info.textContent = `${entry.source  }\n${  time.toLocaleString(undefined, timeOptions)}`;
      };
      ...
      xhr.onload = function() {
          this.onload = null;
          const parser = new DOMParser(), parsed = parser.parseFromString(this.responseText, "text/html"), fromParent = parsed.body;
        ...
        dom.on("#cloudOptionsSubmit", "click", () => {
            submitOptions();
        });
