...
interface UrlSkipStatic {
    safeBase64Map: Record<string, string>;
    safeBase64Replacer: ((s: string) => string) | undefined;
}

export function urlSkip(url: string, blocked: boolean, steps: string[]): string | undefined {
    try {
        ...
        console.warn('[uBR] urlskip: URL processing failed', e);
    }
}

(urlSkip as unknown as UrlSkipStatic).safeBase64Map = {};
(urlSkip as unknown as UrlSkipStatic).safeBase64Replacer = undefined;