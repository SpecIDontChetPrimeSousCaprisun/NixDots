...
vAPI.closePopup = function() {
    // Inside an embedded extension iframe (popup in logger, dashboard child):
    // window.close() / window.open('', '_self').close() may fail or close
    // only the child browsing context without telling the parent page,
    // leaving stale state (watchdog, observers, realTabId) on the parent.
    // Instead, post a message to the parent logger/dashboard so it can
    // clean up properly.
    if ( window !== window.top ) {
        const tabId = parseInt(
            new URL(self.location.href).searchParams.get('tabId'),
            10,
        ) || null;
        window.parent.postMessage({
            source: 'uBlockResurrected',
            type: 'embeddedPopupClose',
            reason: 'popupCommandCompleted',
            tabId,
